...
def copy_frame_array(input_frame, output_frame):
    if input_frame >= totalFrames-1:
        return False
    global outputFiles
    src = "frame{:06d}.jpg".format(input_frame+1)
    while not os.path.isfile(TEMP_FOLDER + "/" + src):
...
totalFrames = int(subprocess.check_output(['ffprobe -v error -select_streams v:0 -count_packets -show_entries stream=nb_read_packets -of csv=p=0 "'+INPUT_FILE+'"'], shell=True).decode('utf-8'))
print("Got", totalFrames, "frames total")
...
vid = subprocess.Popen(["./conv-parallel.sh -i " + INPUT_FILE + " -o " + TEMP_FOLDER + " -of frame%06d.jpg -qscale:v " +
                str(FRAME_QUALITY)], shell=True, stdout=subprocess.DEVNULL)
...
def render_chunk(partial_chunk, thread_num):
    ...
    for chunk in partial_chunk:
        ...
        for output_frame in range(start_output_frame, end_output_frame):
            ...
            if input_frame >= totalFrames-1:
                input_frame = totalFrames-1
                print("done")
                print("done", input_frame, "-", totalFrames)
                print("done")
            did_it_work = copy_frame(input_frame, output_frame)
            if did_it_work:
...
if outputFiles:
    if os.path.exists(TEMP_FOLDER + "/frames.txt"):
        os.remove(TEMP_FOLDER + "/frames.txt")
...
subprocess.call("ffmpeg -hide_banner -loglevel error -y -vsync 0 -hwaccel cuda -hwaccel_output_format cuda -c:v "
                "mjpeg_cuvid -r " + str(frameRate) + " -f concat -i " + TEMP_FOLDER + "/frames.txt -i " +
