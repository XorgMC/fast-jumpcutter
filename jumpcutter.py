...
if(args.prefps != None):
    ...
    subprocess.call("ffmpeg -hide_banner -loglevel error -y -i \""+INPUT_FILE+"\" -filter:v fps="+str(int(args.prefps))+" -c:v libx264 -preset veryfast -c:a copy \""+TEMP_FOLDER+"/temp.mp4\"", shell=True)
...
command = "ffmpeg -hide_banner -loglevel error -y -vsync 0 -hwaccel cuda -hwaccel_output_format cuda -c:v mjpeg_cuvid -framerate "+str(frameRate)+" -i "+TEMP_FOLDER+"/newFrame%06d.jpg -i "+TEMP_FOLDER+"/audioNew.wav -c:a aac -c:v libx264 "+OUTPUT_FILE
subprocess.call(command, shell=True)
