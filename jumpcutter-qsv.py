...
if args.prefps is not None:
    ...
    subprocess.call(
        "ffmpeg -hide_banner -loglevel error -init_hw_device qsv=hw -filter_hw_device hw -y -i \"" + INPUT_FILE +
        "\" -filter:v fps=" + str(int(args.prefps)) + "-vf hwupload=extra_hw_frames=64,format=qsv -c:v h264_qsv -b:v "
                                                      "5M -c:a copy \"" + TEMP_FOLDER + "/temp.mp4\"",  shell=True)
...
subprocess.call("ffmpeg -hide_banner -loglevel error -y -hwaccel qsv -c:v mjpeg_qsv -framerate " + str(
    frameRate) + " -i " + TEMP_FOLDER + "/newFrame%06d.jpg -i " + TEMP_FOLDER + "/audioNew.wav -c:a aac -c:v h264_qsv" \
                                                                                " -b:v 5M " + OUTPUT_FILE, shell=True)
