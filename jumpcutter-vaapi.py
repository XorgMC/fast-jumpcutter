...
parser.add_argument('-va', '--vaapi-device', dest='vadev', default="/dev/dri/renderD128",
                    help="VAAPI Device to use (default: /dev/dri/renderD128)")
...
if args.prefps is not None:
    ...
    subprocess.call("ffmpeg -vaapi_device " + args.vadev + " -hide_banner -loglevel error -y -i \"" + INPUT_FILE + "\" "
                                                                                                                   "-filter:v fps=" + str(
        int(args.prefps)) + "-vf 'format=nv12,hwupload' -c:v h264_vaapi -b:v 5M "
                            "-c:a copy \"" + TEMP_FOLDER + "/temp.mp4\"", shell=True)
    INPUT_FILE = TEMP_FOLDER + "temp.mp4"
    frameRate = args.prefps

if frameRate == -1:
    frameRate = convert_to_float(subprocess.check_output(['ffprobe -v 0 -of csv=p=0 -select_streams v:0 -show_entries '
                                                          'stream=r_frame_rate "' + INPUT_FILE + '"'],
...
if SAMPLE_RATE == -1:
    SAMPLE_RATE = int(subprocess.check_output(['ffprobe -v error -select_streams a -of '
                                               ...
                                               '"' + INPUT_FILE + '"'], shell=True).decode('utf-8'))
    print("Determined sample rate: ", SAMPLE_RATE, "Hz")

p = subprocess.Popen(["ffmpeg -y -i " + INPUT_FILE + " -ab 160k -ac 2 -ar " + str(SAMPLE_RATE) +
                      "-vn -hide_banner -loglevel error " + TEMP_FOLDER + "/audio.wav"], shell=True)
...
subprocess.call("ffmpeg -hide_banner -loglevel error -y -vaapi_device " + args.vadev + " -framerate " + \
                str(frameRate) + " -i " + TEMP_FOLDER + "/newFrame%06d.jpg -i " + TEMP_FOLDER +
                "/audioNew.wav -c:a aac -vf 'format=nv12,hwupload' -c:v h264_vaapi -b:v 5M " + OUTPUT_FILE, shell=True)
